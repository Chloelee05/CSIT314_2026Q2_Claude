...
/**
 * BCE Boundary: ViewActivityBoundary
 * process_view()       → Donee navigates here by clicking a campaign link.
 * show_activity_details() → Rendered below after fetching data from controller.
 */
export default async function ViewActivityPage({
  ...
  const [success, message, activity] = await ViewActivityController.ViewActivity(activity_id);
  ...
  // Exception flow: activity not found
  if (!activity) {
    return (
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {/* Exception flow: closed/completed badge */}
        {isClosed && (
          <div className="mb-4 bg-yellow-50 border border-yellow-200 rounded-xl px-5 py-3 text-yellow-800 font-medium text-sm">
            This campaign is <span className="capitalize font-bold">{activity.status}</span> — {message}
          </div>
        )}
