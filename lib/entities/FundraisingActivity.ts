 ...
 * Merged schema: includes both FR-side fields (user_id, category, view_count)
 * and donee-side fields (raised_amount, status).
 */
export class FundraisingActivity {
  ...
  constructor(data: Record<string, unknown>) {
    ...
    this.goal_amount =
      typeof data.goal_amount === 'string'
        ...
        : (data.goal_amount as number) ?? 0;
    this.raised_amount =
      typeof data.raised_amount === 'string'
        ? parseFloat(data.raised_amount)
        : (data.raised_amount as number) ?? 0;
    this.category = (data.category as string) ?? 'General';
   ...
   * (case-insensitive substring). Empty keyword returns all activities for the user.
   */
  ...
  /**
   * Global search for active fundraising activities by keyword (for donee browsing).
   * Returns a tuple: [success, message, activities]
   */
  static async find_active_activities(
    keyword: string,
  ...
  /**
   * Fetch full details of a single fundraising activity by ID.
  ...
  /**
   * Load a single activity by id only if it is "completed".
   ...
   * Signature matches BCE diagram: save_activity(activity_id, title, description, goal): tuple
   */
