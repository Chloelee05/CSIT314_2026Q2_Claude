...
export default function SuspendUserProfileBoundary({ userprofile_id, currentStatus }: Props) {
  ...
  /* SuspendUserProfile() — submit calls SuspendUserProfileController.SuspendUserProfile(userprofile_id) */
  return (
    <form action={formAction} className="inline-block">
      <input type="hidden" name="userprofile_id" value={userprofile_id} />
      {state.message && (
        <span
          className={`text-xs mr-2 ${state.success ? 'text-green-600' : 'text-red-600'}`}
        >
          {state.message}
        </span>
      )}
