...
export default function ReportsPageBoundary({ report, flash }: Props) {
  ...
  function showWeeklyReport(report_data: WeeklyReport) {
    return (
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-1">
          Week of{' '}
          <span className="text-indigo-600">
            {new Date(report_data.startDate + 'T12:00:00').toLocaleDateString(undefined, {
              month: 'long',
              day: 'numeric',
            })}
          </span>
          {' '}–{' '}
          <span className="text-indigo-600">
            {new Date(report_data.endDate + 'T12:00:00').toLocaleDateString(undefined, {
              year: 'numeric',
              month: 'long',
              day: 'numeric',
            })}
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      {report && showWeeklyReport(report)}
    </div>
  );
}
