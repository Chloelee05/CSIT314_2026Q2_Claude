...
import { updateUserProfileAction, UpdateProfileState } from '@/app/admin/profile/update/actions';
...
/**
 * BCE Boundary: UpdateUserProfileBoundary (User Story #13)
 *
 * - Show_Update_Form()    — renders pre-populated update form
 * - UpdatedUserProfile()  — Save Changes submits to UpdateUserProfileController.UpdatedUserProfile(...)
 */
export default function UpdateUserProfileBoundary({
  ...
  /* Show_Update_Form() — render pre-populated form */
  return (
    <form action={formAction} className="bg-white shadow-sm rounded-xl border border-gray-200 p-8 space-y-6">
      ...
      {/* UpdatedUserProfile() — submit calls UpdateUserProfileController.UpdatedUserProfile(...) */}
      <div className="pt-2">
        <button
          type="submit"
          disabled={isPending}
