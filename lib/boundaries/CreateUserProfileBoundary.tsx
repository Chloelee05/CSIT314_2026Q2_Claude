...
/**
 * BCE Boundary: CreateUserProfileBoundary (User Story #11)
 *
 * - Show_Create_Form()    — renders the create profile form pre-populated with account selector
 * - CreateUserProfile()   — form submission triggers CreateUserProfileController.CreateUserProfile(...)
 */
export default function CreateUserProfileBoundary({ accounts }: Props) {
  const [state, formAction, isPending] = useActionState(createUserProfileAction, initialState);

  /* Show_Create_Form() — render the form */
  return (
    <form action={formAction} className="space-y-5">
      ...
      {/* CreateUserProfile() — submit calls CreateUserProfileController.CreateUserProfile(...) */}
      <div className="pt-2">
        <button
          type="submit"
          disabled={isPending}
